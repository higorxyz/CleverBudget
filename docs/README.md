# 📚 Documentação da API CleverBudget

Bem-vindo à documentação técnica do CleverBudget! Esta pasta contém guias e referências para desenvolvedores que trabalham com a API.

## 📋 Índice de Documentação

### 🚀 Início Rápido
- **[Guia de Configuração](./SETUP.md)** - Como configurar o ambiente de desenvolvimento
- **[Arquitetura do Projeto](./ARCHITECTURE.md)** - Estrutura e organização do código

### 🔐 Autenticação e Segurança
- **[Autenticação](./AUTHENTICATION.md)** - Sistema de autenticação JWT

### 🔧 Serviços Externos
- **[Serviços e Dependências](./EXTERNAL_SERVICES.md)** - Cloudinary, Brevo, QuestPDF, CsvHelper e outros

### 📡 API Reference
- **[Endpoints](./ENDPOINTS.md)** - Lista completa de endpoints disponíveis
- **[Mensagens de Erro](./ERROR_MESSAGES.md)** - Códigos e mensagens de erro

### 💾 Banco de Dados
- **[Schema do Banco](./DATABASE_SCHEMA.md)** - Estrutura das tabelas e relacionamentos

### 🧪 Testes
- **[Guia de Testes](./TESTING.md)** - Como escrever e executar testes

### 🔧 Desenvolvimento
- **[Contribuindo](./CONTRIBUTING.md)** - Como contribuir com o projeto

### 🚢 Deploy
- **[Deploy](./DEPLOYMENT.md)** - Instruções de deploy (Railway, Docker, etc)
- **[Variáveis de Ambiente](./ENVIRONMENT_VARIABLES.md)** - Configurações necessárias

## 🆘 Precisa de Ajuda?

- Encontrou um bug? Abra uma issue
- Tem uma dúvida? Consulte a documentação específica
- Quer contribuir? Veja [CONTRIBUTING.md](./CONTRIBUTING.md)

---

**Última atualização:** Novembro 2025  
**Versão da API:** 1.0.0
